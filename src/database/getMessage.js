...
module.exports = async (channel) => {
	...
	try {
		...
		let messages = await conn.query(`SELECT recent_messages.id AS id, recent_messages.text AS text, recent_messages.channel AS channel, JSON_OBJECT('id', users.userId,'username', users.username) AS user FROM (SELECT id, text, channel, user FROM messages WHERE channel=? ORDER BY id DESC LIMIT 40 ) AS recent_messages JOIN users ON recent_messages.user = users.userId ORDER BY recent_messages.id ASC`, [channel]);
